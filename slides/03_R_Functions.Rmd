...
- Surround weird a function/variable name with backquotes to access it
.center[
...
- Replacement are done via replacement functions.
    - E.g., `` `class<-` ``
...
- `invisible(x)` returns `x` but do not print. Useful for 
    ```{r, eval=FALSE}
    # not real code
...
Suppose the dataset is given by `pscl::prussian$y`, yearly number of Prussian solders kicked dead by army hourses. Manufacture the likelihood for this dataset:
