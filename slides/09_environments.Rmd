...
- Environments are not copied when modified. Names bound to the same environment object will always operate on the same object without copying it.
    ```{r, error=TRUE}
    f <- function(e) {
      e$a <- 100
      stop("Don't return")
...
```{r}
env_print(e1)
e1$anEnv <- env(e1, d=1) # Unnamed input to `env()` specifies the parent
env_parent(e1$anEnv)
```
...
- The *empty environment* is the only environment that does not have a parent
...
- Create a new environment `env2` which should have parent being `env1`. Add a variable `a = 2` into `env2` (`env(env1, var=value)` does this)
...
- avoid accidental copies of large data (actually, when a list is copied, R usually make a shallow copy of only the references to the list elements so that is not super costly either)
