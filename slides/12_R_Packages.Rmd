...
- The file `NAMESPACE` controls the 
    ...
    - all functions including unexported helper functions are bound in the namespace:pkg environment
...
- Use `devtools::document()` to compile the documents and update the NAMESPACE, and then `devtools::load_all()` again. You will then be able to find `?func`
...
- `@param mod1,mod2 Two lm models to be compared` 
...
- Repeating documentation is good for the users. This is sometimes inevitable because functions behave very similarly
...
- `@inherit`, `@inheritParams`: Inherit a part of the documentation for another function. [See `str_replace`](https://github.com/tidyverse/stringr/blob/HEAD/R/replace.r)
...
- `tests/testthat.R` is a boilerplate code for running all the tests
...
.center[
![Package states](https://r-pkgs.org/diagrams/installation.png)
