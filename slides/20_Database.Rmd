...
- The `IN` AND `EXISTS` operations are often useful in combination with subqueries
...
These approaches all creates a table-like object for intermediate use (e.g., break down a large query):

- `CREATE TABLE` writes a new table on the harddisk

- `CREATE TEMPORARAY TABLE` is similar to `CREATE TABLE`, except that the temporary table will be removed after your SQL session ends. The temporary table exists on the harddisk
...
- `WITH tab AS (subquery) SELECT ... FROM tab` creates an auxilliary statement that works like a table that can be used in the subsequent query
    - This approach does not require write access to the database, while the temp table and view approaches do

---

## Advanced: Window functions

- Some queries are awkward when written using the techniques covered before, or even just show the top 2 rows for each ethnicity group in `admissionsMini`
...
- A *window function* performs calculation over a range of rows related to the current one. It is great for computing something that depends on the order of data, e.g., 
    - a moving average 
    - whether an admission has the same diagnosis as the last admission
    - ranks
...
- Window functions returns one result per row (like `dplyr::mutate()`), while aggregators return one result per group (like `dplyr::summarize()`)
...
- `OVER`: Specify the window for the window function
    ...
    - `ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING`: Constraint rows in the window that is up to the current row (inclusive)
    - `RANGE BETWEEN 1000 PRECEDING AND 1000 FOLLOWING`: Constraint rows according to the value in the order by column
...
<!-- - I made available the full MIMIC 3 database through Google BigQuery [here](https://console.cloud.google.com/bigquery?project=mimic-3-ph-290&ws=!1m4!1m3!3m2!1smimic-3-ph-290!2smimic3_mod) for you to practice (and for HW). These data are slightly perturbed off the original version -->

<!-- - Your Berkeley accounts are given access to this project. Make sure to you are accessing the project using the correct account -->
