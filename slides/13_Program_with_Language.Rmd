...
1. Turn that into a string (`deparse1()`)
...
- `eval_tidy()` is like `eval()` but uses the most relevant context to evaluate when it sees a quosure
...
- This is most often used to capture the input by user and later use it for evaluation
...
- No matter in what environment do you evaluate this, you get the right `xval`
```{r}
f2 <- function(x) {
  xval <- 3
...
- This is called injection. It is most commonly done using `!!` (similar to `substitute()`ing the values)
...
- The `{{` (embrace) operator let you pass through an NSE argument (preferred by RStudio)
